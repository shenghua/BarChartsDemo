...
public protocol ChartViewDelegate
{
    ...
    @objc optional func chartDefaultValueSelected(_ chartView: ChartViewBase, entry: ChartDataEntry, highlight: Highlight)
    
    @objc optional func chartValueSelected(_ chartView: ChartViewBase, entry1: ChartDataEntry, highlight1: Highlight, entry2: ChartDataEntry, highlight2: Highlight)
...
open class ChartViewBase: NSUIView, ChartDataProvider, AnimatorDelegate
{
    ...
    /// add by ledka
    open var isDefaultHighlight: Bool = false
    ...
    /// call delegate method chartValueSelected with no redraw - add by ledka
    open func defaultHighlight(_ highlight: Highlight?, callDelegate: Bool)
    {
        ...
        {
            // notify the listener
            delegate!.chartDefaultValueSelected?(self, entry: entry!, highlight: h!)
    ...
    /// Highlights the value selected by touch gesture. - add by ledka
    open func highlightValue(_ highlight1: Highlight?, _ highlight2: Highlight?, callDelegate: Bool)
    {
        ...
        {
            // set the indices to highlight
            entry1 = _data?.entryForHighlight(h1!)
            if entry1 == nil
            {
                h1 = nil
                _indicesToHighlight.removeAll(keepingCapacity: false)
        ...
        {
            // set the indices to highlight
            entry2 = _data?.entryForHighlight(h2!)
            if entry2 == nil
            {
                h2 = nil
                _indicesToHighlight.removeAll(keepingCapacity: false)
        ...
        if callDelegate && delegate != nil
        {
            ...
            {
                // notify the listener
                delegate!.chartValueSelected?(self, entry1: entry1!, highlight1: h1!, entry2: entry2!, highlight2: h2!)
            }
        }
