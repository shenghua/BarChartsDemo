...
open class BarLineChartViewBase: ChartViewBase, BarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
    ...
    open var gridBackgroundColor = NSUIColor(red: 243/255.0, green: 246/255.0, blue: 252/255.0, alpha: 1.0)
    ...
    //双指单击
    internal var _twofingerTapOneGestureRecognizer: NSUITapGestureRecognizer!
    ...
    internal override func initialize()
    {
        ...
        //单指单击
        _twofingerTapOneGestureRecognizer = NSUITapGestureRecognizer(target: self, action: #selector(tapGestureRecognized(_:)))
        _twofingerTapOneGestureRecognizer.nsuiNumberOfTapsRequired = 1
        _twofingerTapOneGestureRecognizer.numberOfTouchesRequired = 2
        ...
        // remove GestureRecognizer while user touch instand - changed by ledka
//        self.addGestureRecognizer(_tapGestureRecognizer)
//        self.addGestureRecognizer(_twofingerTapOneGestureRecognizer)
//        self.addGestureRecognizer(_doubleTapGestureRecognizer)
//        self.addGestureRecognizer(_panGestureRecognizer)
        
        _doubleTapGestureRecognizer.isEnabled = _doubleTapToZoomEnabled
//        _doubleTapGestureRecognizer.isEnabled = true
...
//            _pinchGestureRecognizer.isEnabled = _pinchZoomEnabled || _scaleXEnabled || _scaleYEnabled
            _pinchGestureRecognizer.isEnabled=false;
        #endif
    }
...
//        _legendRenderer.renderLegend(context: context)
        ...
        // add by ledka
        if (_indicesToHighlight.count > 0 && isDefaultHighlight) {
            self.defaultHighlight(_indicesToHighlight.first, callDelegate: true)
        }
    }
    ...
    open override func nsuiTouchesBegan(_ touches: Set<NSUITouch>, withEvent event: NSUIEvent?) {
        ...
        // max touch point is 2
        if totalTouch == 2 {
            return;
        }
        
        totalTouch += touches.count;
        self.handleTouchEvent(self .removeTouchOutside((event?.allTouches)!))
    }
    ...
    open override func nsuiTouchesEnded(_ touches: Set<NSUITouch>, withEvent event: NSUIEvent?) {
        ...
        // all touches in screen
        var tempTouches = Set<UITouch>()
        tempTouches = (event?.allTouches)!
    
        // remove touch end point in this view
        for touch in touches {
            tempTouches.remove(touch);
            totalTouch -= 1;
        }
        
        self.handleTouchEvent(self .removeTouchOutside(tempTouches))
    }
    
    // remove touch point outside this view
    func removeTouchOutside(_ touches: Set<UITouch>) -> Set<UITouch> {
        var tempTouches = touches
        
        for touch in touches {
            var touchLocation = touch.location(in: self);
            if touchLocation.x < 0 || touchLocation.y < 0 || touchLocation.x > self.bounds.width || touchLocation.y > self.bounds.height {
                tempTouches.remove(touch);
            }
        }
    ...
    func handleTouchEvent(_ touches: Set<NSUITouch>) {
        
        // one figure
        if (touches.count == 1) {
            let touch = touches.first as NSUITouch!
            var touchLocation = touch?.location(in: self)
            // default select top section
            touchLocation!.y = 0.0
            
            let h = getHighlightByTouchPoint(touchLocation!)
        ...
        // multi figure
            ...
            let touch1 = touches.first as NSUITouch!
            var touchLocation1 = touch1?.location(in: self)
            touchLocation1!.y = 0.0
            
            let h1 = getHighlightByTouchPoint(touchLocation1!)
            if h1 === nil || h1!.isEqual(self.lastHighlighted)
            {
                self.lastHighlighted = nil
            }
            else
            {
                self.lastHighlighted = h1
            }
            
            let touch2 = touches[touches.index(touches.startIndex, offsetBy: 1)];
            var touchLocation2 = touch2.location(in: self)
            touchLocation2.y = 0.0
            
            let h2 = getHighlightByTouchPoint(touchLocation2)
            if h2 === nil || h2!.isEqual(self.lastHighlighted)
            {
                self.lastHighlighted = nil
            }
            else
            {
                self.lastHighlighted = h2
            }
            
            self.highlightValue(h1, h2, callDelegate: true)
        }
        // nothing touched
        else {
            totalTouch = 0;
...
//    }
    ...
    {
        NSLog("tapGestureRecognized %d",recognizer.nsuiNumberOfTouches());
        if _data === nil
        {
            return
        }
        ...
        if recognizer.state == NSUIGestureRecognizerState.ended
        {
            ...
            }else//双指单击
            {
                ...
                //for touchindex in 0..<recognizer.nsuiNumberOfTouches()
               // {
                    
                        //                let h = getHighlightByTouchPoint(recognizer.location(in: self))
                    //第一个手指
                        let h1 = getHighlightByTouchPoint(recognizer.location(ofTouch: 0, in: self))
                        if h1 === nil || h1!.isEqual(self.lastHighlighted)
                        {
//                            self.highlightValue(nil, callDelegate: true)
                            self.lastHighlighted = nil
                        ...
                        {
//                            self.highlightValue(h1, callDelegate: true)
                            self.lastHighlighted = h1
                        }
                     //第二个手指
                        let h2 = getHighlightByTouchPoint(recognizer.location(ofTouch: 1, in: self))
                        if h2 === nil || h2!.isEqual(self.lastHighlighted)
                        {
//                            self.highlightValue(nil, callDelegate: true)
                            self.lastHighlighted = nil
                        ...
                        {
//                            self.highlightValue(h2, callDelegate: true)
                            self.lastHighlighted = h2
                        }
    ...
    {
        NSLog("doubleTapGestureRecognized");
        if _data === nil
        {
            return
        }
    ...
    {
        NSLog("panGestureRecognized %d",recognizer.nsuiNumberOfTouches());
